...
while True:
    ...
    # Use apply_chat_template with add_generation_prompt=True for instruct models
    input_ids = tokenizer.apply_chat_template(
        chat_history,
        tokenize=True,
        add_generation_prompt=True, # Important for instruct models to prompt for assistant response
        return_tensors="pt"
    ).to(device)

    # Generate a response
    # max_new_tokens: controls the length of the *new* generated text, not total length
    # num_beams, length_penalty, no_repeat_ngram_size, do_sample, top_k, top_p, temperature:
    # these are generation parameters for controlling output quality and diversity.
    generated_ids = model.generate(
        input_ids,
        max_new_tokens=256, # Generate up to 256 new tokens for the bot's response
        ...
        top_p=0.95,
        temperature=0.7,
        # stopping_criteria=StoppingCriteriaList([StopOnTokens(tokenizer.eos_token_id)]) # Optional: custom stopping
    )

    # Decode the generated response
    # We only want the new part generated by the model, not the input prompt
